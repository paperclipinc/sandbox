...
4. **The secondary hardening** below: the raw-forkd shared-rootfs channel and
   the raw-forkd privileged DaemonSet remain. Three earlier secondary items are
   now SHIPPED: the husk pod no longer automounts the default SA token, the forkd
   gRPC surface fails closed without mTLS (opt-in `--allow-insecure-grpc` for
   dev), and the host-side vsock client applies a per-request read deadline.
...
a KVM-capable kubelet. The raw-forkd nftables egress IS CI-proven in-VM on KVM
